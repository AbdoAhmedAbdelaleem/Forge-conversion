 ...
 */
function* parseFragments(buffer, fragmentOffset = { x: 0, y: 0, z: 0 }) {
    ...
    while (stream.offset < stream.length - 1) {
        ...
        const dbId = idata[2];
        const flags = idata[3];
        const translation = { x: fdata[4] + fragmentOffset.x, y: fdata[5] + fragmentOffset.y, z: fdata[6] + fragmentOffset.z };
        const quaternion = { x: fdata[7], y: fdata[8], z: fdata[9], w: fdata[10] };
        const scale = { x: fdata[11], y: fdata[12], z: fdata[13] };
        yield {
            geomId,
            materialId,
            dbId,
